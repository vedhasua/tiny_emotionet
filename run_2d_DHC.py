...
def main():
    # Configuration
    if True:
        modelFile          = '/home/vedhas/workspace/is2019_recurrence/DHCresults/2/m1000_tr0.687_dv0.608_ts0.625_tc0.420.h5' #'/home/vedhas/workspace/is2019_recurrence/results/8_oV_iv/m498_tr0.647_dv0.560_ts0.581_tc0.375.h5'
        culture            = 'German'
        eval_cross         = True
        modality           = 'video'
    ...
    if eval_cross:
        ...
        if os.path.isfile(CnNPY) and False:
    ...
    print(model.summary(),trainCX.shape)

    model2 = convert_to_2dpure(model)

    out1 = model.predict(testCX)
    out2 = model2.predict(np.expand_dims(testCX, axis=-1))
    plt.plot(out1[0].flatten(),'b')
    plt.plot(out2[0][:,18,:].flatten(),'r*')
    plt.show()
    ...
    if True:
        ...
        if eval_cross:
            CCC_train, CCC_devel, CCC_test, CCC_testT1, CCC_orig_train, CCC_orig_devel, CCC_orig_test, CCC_orig_testT1, CCC_orig_train_seq, CCC_orig_devel_seq, CCC_orig_test_seq, CCC_orig_test_seqT1, CCC_orig_devel_pp, CCC_orig_test_pp, CCC_orig_test_ppT1 = evaluate_all_cross(model, trainX, develX, testX, crossX, trainY, develY, testY, crossY, origTrain, origDevel, origTest, origCross, shift, factor_tr_orig, num_targets)
            CCCs          = np.append(CCCs,         [np.array([CCC_train,          CCC_devel,          CCC_test,          CCC_testT1]).flatten()],          axis=0)
            CCCs_orig     = np.append(CCCs_orig,    [np.array([CCC_orig_train,     CCC_orig_devel,     CCC_orig_test,     CCC_orig_testT1]).flatten()],     axis=0)
            CCCs_orig_seq = np.append(CCCs_orig_seq,[np.array([CCC_orig_train_seq, CCC_orig_devel_seq, CCC_orig_test_seq, CCC_orig_test_seqT1]).flatten()], axis=0)
            CCCs_orig_pp  = np.append(CCCs_orig_pp, [np.array([CCC_orig_devel_pp,  CCC_orig_test_pp,   CCC_orig_test_ppT1                    ]).flatten()], axis=0)
            #####
